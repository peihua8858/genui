      ...
      if (event is ConversationContentReceived) {
        ...
        if (event.text.isNotEmpty) {
          ...
          if (updatedMessages.isNotEmpty &&
              ...
              !updatedMessages.last.parts.any((p) => p is DataPart && p.isUiPart)) {
            updatedMessages.last.parts.whereType<TextPart>().map((part) => part.text).last.value += event.text;
          }else{
        ...
        updatedMessages.add(
          ChatMessage(
            role: .model,
            parts: [UiPart.create(definition: event.definition, surfaceId: event.surfaceId)],
          ),
        );
        _messages.value = updatedMessages;
...
''',
  PromptFragments.uiGenerationRestriction(prefix: PromptBuilder.defaultImportancePrefix),
];
